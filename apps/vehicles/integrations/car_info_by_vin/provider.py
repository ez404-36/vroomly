from apps.vehicles.integrations.car_info_by_vin.schema import CarInfoByVIN, CarInfoByVinData
from core.providers.vin01 import VinO1ApiProvider, Vin01ApiException
...
class CarInfoByVinProvider(VinO1ApiProvider):
    ...
    def get_info(self, vin: str) -> CarInfoByVinData:
        ...
        if response.status != 200 or not response.success:
            raise Vin01ApiException(f'Ошибка получения данных из сервиса {self.base_url}')
