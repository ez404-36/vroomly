...
class _NodeIndex:
    ...
    async def load(self, session: AsyncSession) -> None:
        ...
        for engine in engines:
            ...
            if engine.concern_id is not None:
                self.engine_by_concern.setdefault(
                    (engine.concern_id, engine.name), engine,
                )
        ...
        logger.info(
            'Loaded: brands=%d, series=%d, generations=%d, engines=%d, '
            'transmissions=%d, existing_trims=%d',
            len(self.brand_by_id), len(self.series_ids),
            len(self.generation_by_key), len(engines), len(transmissions),
...
class _Importer:
    ...
    def _process_one(self, entry: dict[str, Any]) -> None:
        ...
        dedup_key = (
            generation.id, engine.id, transmission.id, trim_name, body_str,
        )
...
async def main() -> None:
    ...
    async with database.get_async_session() as session:
        ...
        _write_missing_nodes_json(
            importer.still_unresolved, MISSING_ENGINES_JSON_PATH,
            reason='engine_not_found',
            candidates_key='engine_candidates',
            label='двигателей',
        )
        _write_missing_nodes_json(
            importer.still_unresolved, MISSING_TRANSMISSIONS_JSON_PATH,
