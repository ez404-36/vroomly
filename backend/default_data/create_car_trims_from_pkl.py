...
class CarTrimBuilder:
	...
	async def prefetch(self) -> None:
		...
		logger.info(
			...
			len(self._brand_by_code),
			len(self._series_by_brand_name),
			len(engines),
			len(transmissions),
		)
		...
		for entry in entries:
			...
			if dedup_key in self._generation_keys:
				...
				generation = next(
					...
					if g.series_id == series.id and g.name == gen_name and g.is_restyling == is_restyling
				)
			else:
			...
			self.unresolved.append(
				{
					...
					'concern_id': (str(brand.concern_id) if brand.concern_id is not None else None),
					'generation': {
...
async def main() -> None:
	...
	logger.info(
		...
		len(raw_responses) - parse_failures,
		len(raw_responses),
		parse_failures,
		len(parsed_entries),
	)
