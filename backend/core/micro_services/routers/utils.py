...
def get_default_router(prefix: str, tag_name: str | None = None) -> APIRouter:
	"""Создаёт ``APIRouter`` с префиксом и тегом по умолчанию равным ``prefix``."""
...
def register_all_service_routers(_root_router: APIRouter | FastAPI) -> None:
	"""Автоматически регистрирует все роутеры из ``apps/**/routers`` в root-приложении."""
	...
	for router_file in all_router_files:
		...
		try:
			...
			logger.error(f'Не удалось импортировать роутеры из файла {router_file} по причине: {e}')
			continue
		except Exception as e:
			logger.error(e)
			continue
