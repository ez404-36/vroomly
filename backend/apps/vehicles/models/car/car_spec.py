...
class CarSpec(
    ...
    Связи:
        ...
        - ``trim`` — N:1 с CarTrim (заводская комплектация). Через ``trim``
          доступны generation, series, brand, concern.
        - ``engine`` — N:1 с VehicleEngine, **опционально**: NULL ⇒
          фактический двигатель совпадает с ``trim.engine``; не-NULL ⇒
          двигатель экземпляра отличается от заводского (свап).
        - ``transmission`` — N:1 с CarTransmission, **опционально**:
          аналогично engine.
