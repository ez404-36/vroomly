...
class UserVehicleAPI(BaseAPI):
	...
	async def list_user_vehicles(
		...
		query = (
			select(UserVehicle).where(UserVehicle.user_id == self.user.id).options(selectinload(UserVehicle.vehicle))
		)
	...
	async def get_user_vehicle(
		...
		query = (
			...
			.options(
				...
				selectinload(UserVehicle.generation)
				.selectinload(UserVehicle.__mapper__.relationships['generation'].entity.class_.series)
				.selectinload(
